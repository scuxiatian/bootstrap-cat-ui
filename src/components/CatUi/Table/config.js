...
export const cellStarts = {
  ...
  selection: {
    ...
    realWidth: 48,
    order: '',
    className: 'cat-table-column--selection'
  },
  expand: {
    width: 48,
    minWidth: 48,
...
export const cellForced = {
  ...
  expand: {
    ...
    renderCell: function (h, { row, store }) {
      const classes = ['cat-table__expand-icon']
      if (store.states.expandRows.indexOf(row) > -1) {
        classes.push('cat-table__expand-icon--expanded')
      }
      ...
      return (<div class={ classes }
        on-click={callback}>
        <i class='cat-icon cat-icon-arrow-right'></i>
      </div>)
    },
    sortable: false,
    resizable: false,
    className: 'cat-table__expand-column'
  }
}
