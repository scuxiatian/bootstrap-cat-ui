...
import { noop, isEmpty, valueEquals, isEqual, coerceTruthyValueToArray } from '../utils/util'
...
const getSibling = (el, distance) => {
  const { parentNode } = el
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.cat-cascader-node[tabindex="-1"]')
...
export default {
  ...
  methods: {
    ...
    expandNodes (nodes) {
      nodes.forEach(node => this.handleExpand(node, true))
    },
    ...
    handleKeyDown (e) {
      ...
      let prev, next, preMenu, expandedNode, nextMenu, firstNode

      switch (keyCode) {
        case KeyCode.up:
          prev = getSibling(target, -1)
          focusNode(prev)
          break
        case KeyCode.down:
          next = getSibling(target, 1)
          focusNode(next)
          break
        case KeyCode.left:
          preMenu = this.$refs.menu[getMenuIndex(target) - 1]
          if (preMenu) {
            expandedNode = preMenu.$el.querySelector('.cat-cascader-node[aria-expanded="true"]')
            focusNode(expandedNode)
          }
          break
        case KeyCode.right:
          nextMenu = this.$refs.menu[getMenuIndex(target) + 1]
          if (nextMenu) {
            firstNode = nextMenu.$el.querySelector('.cat-cascader-node[tabindex="-1"]')
            focusNode(firstNode)
          }
          break
        case KeyCode.enter:
    ...
    scrollIntoView () {
      ...
      menus.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.cat-scrollbar__wrap')
          const activeNode = menuElement.querySelector('.cat-cascader-node.is-active') ||
            menuElement.querySelector('.cat-cascader-node.in-active-path')
